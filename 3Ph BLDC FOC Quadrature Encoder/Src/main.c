...
/* USER CODE BEGIN PV */
/*float targetPosition = 0.0f;  // Target position in radians*/
/*volatile bool startMotor = false;      // Global flag to control the motor state*/
/* USER CODE END PV */
...
/* Global variables */
float targetPosition = 0.0f;  // Global variable for motor target position
bool startMotor = false;      // Global flag to start the motor
...
int main(void)
{
  ...
  /* Set initial target position and start motor flag */
...
//	  MC_StartMotor1();
//	  if (MC_StartMotor1()) {
//		qd_t initialCurrent = {0}; // Define based on your motor specs
//		MC_SetCurrentReferenceMotor1(initialCurrent);
//		MC_ProgramPositionCommandMotor1(targetPosition, 0); // Immediate movement
//
//		while (MC_GetControlPositionStatusMotor1() != TC_TARGET_POSITION_REACHED) {
//		  osDelay(10);  // Check every 10 ms
//		}
//
//		MC_StopMotor1();
//	  }
